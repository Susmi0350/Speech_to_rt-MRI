...
class Wav2Vec2Model(Wav2Vec2Model):
    ...
    def forward(
        ...
        # Avoid setting this here; already handled in config setup in main.py
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict

        # ✅ FIX: Unwrap tuple if returned
        #features = self.feature_extractor(input_values)[0]
        #if isinstance(features, tuple):
           # features = features[0]
        #hidden_states = features.transpose(1, 2)

        features = self.feature_extractor(input_values)

        # In case it returns a tuple (it often does)
        if isinstance(features, tuple):
          features = features[0]

        # Check shape before transpose
        if features.ndim != 3:
          raise ValueError(f"Expected 3D features, but got shape {features.shape}")

        # Safe to transpose now
        hidden_states = features.transpose(1, 2)  # (B, C, T) -> (B, T, C)


        # Apply dataset-specific interpolation
        if dataset == "BIWI":
        ...
        elif dataset == "rt-mri" or dataset == "4-subjects":
            hidden_states = linear_interpolation(hidden_states, 50, 83, output_len=frame_num)

        # Generate attention mask
        if attention_mask is not None:
            output_lengths = self._get_feat_extract_output_lengths(attention_mask.sum(-1))
            attention_mask = torch.zeros(hidden_states.shape[:2], dtype=hidden_states.dtype, device=hidden_states.device)
            attention_mask[
